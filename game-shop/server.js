...
app.post('/api/admin/products', requireAdmin, (req, res) => {
  const { name, type, game, description, priceBaht, compareAtPriceBaht, isHot, stock, codes } = req.body || {};
  ...
  const codeList = Array.isArray(codes) ? codes.filter(Boolean) : [];
  const info = db.prepare(`
    INSERT INTO products (name, type, game, description, price, compare_at_price, is_hot, stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(name, type, game, description || '', price, compareAtPrice, isHot ? 1 : 0, codeList.length || parseInt(stock, 10) || 0);
