...
app.get('/api/products', (req, res) => {
  ...
  let sql = 'SELECT id, name, type, game, subcategory, description, price, compare_at_price, is_hot, stock, image_url FROM products WHERE active = 1';
  const params = [];
...
// รายการหมวดหมู่ (จัดกลุ่มตามชื่อเกม) พร้อมจำนวนหมวดหมู่ย่อย, จำนวนสินค้า และช่วงราคา — ใช้แสดงเป็นการ์ดหมวดหมู่หน้าแรก
app.get('/api/categories', (req, res) => {
  const rows = db.prepare(`
    SELECT game,
      COUNT(*) AS productCount,
      COUNT(DISTINCT NULLIF(subcategory, '')) AS subcategoryCount,
...
app.post('/api/admin/products', requireAdmin, upload.single('image'), (req, res) => {
  const { name, type, game, subcategory, description, priceBaht, compareAtPriceBaht, isHot, stock } = req.body || {};
  ...
  const info = db.prepare(`
    INSERT INTO products (name, type, game, subcategory, description, price, compare_at_price, is_hot, stock, image_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(name, type, game, (subcategory || '').trim() || null, description || '', price, compareAtPrice, isHotVal, codeList.length || parseInt(stock, 10) || 0, imageUrl);
...
app.post('/api/admin/banners', requireAdmin, upload.single('image'), (req, res) => {
  const { title, subtitle, linkUrl, sortOrder } = req.body || {};
  if (!title) return res.status(400).json({ error: 'กรอกหัวข้อแบนเนอร์' });
  const imageUrl = req.file ? `/uploads/${req.file.filename}` : null;
  const info = db.prepare(`
    INSERT INTO banners (title, subtitle, image_url, link_url, sort_order) VALUES (?, ?, ?, ?, ?)
  `).run(title, subtitle || '', imageUrl, linkUrl || '', parseInt(sortOrder, 10) || 0);
  res.json({ ok: true, bannerId: info.lastInsertRowid });
});

app.post('/api/admin/banners/:id/delete', requireAdmin, (req, res) => {
...
app.get('/api/reviews', (req, res) => {
  const reviews = db.prepare('SELECT id, username, rating, comment, created_at FROM reviews ORDER BY created_at DESC').all();
...
app.post('/api/reviews', requireLogin, (req, res) => {
  ...
  if (!r || r < 1 || r > 5) return res.status(400).json({ error: 'กรุณาให้คะแนน 1-5 ดาว' });
  const user = db.prepare('SELECT username FROM users WHERE id = ?').get(req.session.userId);
  db.prepare('INSERT INTO reviews (user_id, username, rating, comment) VALUES (?, ?, ?, ?)')
    .run(req.session.userId, user.username, r, (comment || '').trim());
  res.json({ ok: true });
});

// ================== ข้อมูลติดต่อร้าน / ตั้งค่า ==================
const CONTACT_KEYS = ['lineId', 'facebookUrl', 'phone', 'businessHours'];
